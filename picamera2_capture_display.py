...
if DATA_DIR not in os.sys.path:
...
def visualize(
    ...
    Args:
        ...
        poses (List): List of pose results, each contianing bounding box, landmarks, mask, etc.

        Returns:
            Tuple[np.ndarray, np.ndarray]: (2D visualiation image, 3D visualization image)
    """
    # Constants for display sizesand colors
    DISPLAY_SIZE = 400
    DISPLAY_CENTER = 200
    SCALE = 100
    ...
    def draw_skeleton_lines(canvas, landmarks, keep_landmarks, thickness: int):
        ...
        connections = [
            ...
            (28,20), (28,32), (30,32), (23,25), (25,27), (27,31),
            (27,29), (29,31)
        ]
        ...
        def draw_edges_on_mask(mask, display_screen):
            """Draws mask edges on the display screen."""
        ...
        def draw_pose_2d(display_screen, bbox, conf, landmarks_screen, keep_landmarks):
               """Draws 2D poses bounding box, confidence, skeleton, and keypoints."""
               bbox = bbox.astype(np.int32)
               cv2.rectangle(display_screen, tuple(bbox[0]), tuple(bbox[1]), COLOR_GREEN, max(1, line_thickness - 1))
               cv2.putText(
               display_screen,
               "{:.4f}".format(conf),
               ...
               draw_skeleton_lines(display_screen, landmarks_xy, keep_landmarks, thickness=max(1, line_thinkness))
               ...
               def draw_pose_3d(display_3d, landmarks_world, keep_landmarks):
                   ...
                   display_3D = None
                   if draw_3d:
                       display_3D = np.zeros((DISPLAY_CENTER, DISPLAY_SIZE, 3), np.uint8)
                       ...
                       for pose_result in poses:
                           ...
                           # Draw 2D pose (bouncding box, skeleton, keypoints)
